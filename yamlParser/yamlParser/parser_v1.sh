#!/bin/bash
#  Example usage ./yaml_parser.sh ../infra kind Deployment
...
process_yaml() {
    ...
    # Use yq to filter YAML documents based on the key and value
    local result=$(yq eval "select(.${key_path} == \"$value\")" "$yaml_file")
